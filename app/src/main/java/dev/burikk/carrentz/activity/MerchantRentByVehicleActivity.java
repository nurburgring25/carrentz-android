...
import dev.burikk.carrentz.adapter.MerchantRentByVehicleAdapter;
...
public class MerchantRentByVehicleActivity extends AppCompatActivity implements MainProtocol<RentByVehicleResponse>, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        super.setContentView(R.layout.activity_merchant_rent_by_vehicle);
    ...
    @Override
    public void result(RentByVehicleResponse data) {
        MerchantRentByVehicleAdapter merchantRentByVehicleAdapter = (MerchantRentByVehicleAdapter) this.recyclerView.getAdapter();

        if (merchantRentByVehicleAdapter != null) {
            merchantRentByVehicleAdapter.getRentByVehicleItems().clear();
            merchantRentByVehicleAdapter.getRentByVehicleItems().addAll(data.getRentByVehicleItems());
            merchantRentByVehicleAdapter.notifyDataSetChanged();

            if (!merchantRentByVehicleAdapter.getRentByVehicleItems().isEmpty()) {
                this.notEmpty();
            } else {
                this.empty();
            }
...

        {
            ...
            for (RentByVehicleItem rentByVehicleItem : data.getRentByVehicleItems()) {
                pieEntries.add(new PieEntry(rentByVehicleItem.getDuration(), rentByVehicleItem.getName(), rentByVehicleItem));
            }
        ...
        this.materialDatePicker.addOnPositiveButtonClickListener(selection -> {
            if (selection != null) {
                if (selection.first != null) {
                    MerchantRentByVehicleActivity.this.cldFrom.setTimeInMillis(selection.first);
                    MerchantRentByVehicleActivity.this.cldFrom.set(Calendar.HOUR, 0);
                    MerchantRentByVehicleActivity.this.cldFrom.set(Calendar.MINUTE, 0);
                    MerchantRentByVehicleActivity.this.cldFrom.set(Calendar.SECOND, 0);
                    MerchantRentByVehicleActivity.this.cldFrom.set(Calendar.MILLISECOND, 0);
                }

                if (selection.second != null) {
                    MerchantRentByVehicleActivity.this.cldUntil.setTimeInMillis(selection.second);
                    MerchantRentByVehicleActivity.this.cldUntil.set(Calendar.HOUR, 0);
                    MerchantRentByVehicleActivity.this.cldUntil.set(Calendar.MINUTE, 0);
                    MerchantRentByVehicleActivity.this.cldUntil.set(Calendar.SECOND, 0);
                    MerchantRentByVehicleActivity.this.cldUntil.set(Calendar.MILLISECOND, 0);
                }
    ...
    private void widget() {
        ...
        this.recyclerView.setAdapter(new MerchantRentByVehicleAdapter(this));
    ...
    private void dateChanged() {
        ...
        this.disposable = MerchantApi.rentByVehicles(
                this,
