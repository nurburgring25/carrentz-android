...
import dev.burikk.carrentz.adapter.MerchantRentByVehicleTypeAdapter;
...
public class MerchantRentByVehicleTypeActivity extends AppCompatActivity implements MainProtocol<RentByVehicleTypeResponse>, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        super.setContentView(R.layout.activity_merchant_rent_by_vehicle_type);
    ...
    @Override
    public void result(RentByVehicleTypeResponse data) {
        MerchantRentByVehicleTypeAdapter merchantRentByVehicleTypeAdapter = (MerchantRentByVehicleTypeAdapter) this.recyclerView.getAdapter();

        if (merchantRentByVehicleTypeAdapter != null) {
            merchantRentByVehicleTypeAdapter.getRentByVehicleTypeItems().clear();
            merchantRentByVehicleTypeAdapter.getRentByVehicleTypeItems().addAll(data.getRentByVehicleTypeItems());
            merchantRentByVehicleTypeAdapter.notifyDataSetChanged();

            if (!merchantRentByVehicleTypeAdapter.getRentByVehicleTypeItems().isEmpty()) {
                this.notEmpty();
            } else {
                this.empty();
            }
...

        {
            ...
            for (RentByVehicleTypeItem rentByVehicleTypeItem : data.getRentByVehicleTypeItems()) {
                pieEntries.add(new PieEntry(rentByVehicleTypeItem.getDuration(), rentByVehicleTypeItem.getName(), rentByVehicleTypeItem));
            }
        ...
        this.materialDatePicker.addOnPositiveButtonClickListener(selection -> {
            if (selection != null) {
                if (selection.first != null) {
                    MerchantRentByVehicleTypeActivity.this.cldFrom.setTimeInMillis(selection.first);
                    MerchantRentByVehicleTypeActivity.this.cldFrom.set(Calendar.HOUR, 0);
                    MerchantRentByVehicleTypeActivity.this.cldFrom.set(Calendar.MINUTE, 0);
                    MerchantRentByVehicleTypeActivity.this.cldFrom.set(Calendar.SECOND, 0);
                    MerchantRentByVehicleTypeActivity.this.cldFrom.set(Calendar.MILLISECOND, 0);
                }

                if (selection.second != null) {
                    MerchantRentByVehicleTypeActivity.this.cldUntil.setTimeInMillis(selection.second);
                    MerchantRentByVehicleTypeActivity.this.cldUntil.set(Calendar.HOUR, 0);
                    MerchantRentByVehicleTypeActivity.this.cldUntil.set(Calendar.MINUTE, 0);
                    MerchantRentByVehicleTypeActivity.this.cldUntil.set(Calendar.SECOND, 0);
                    MerchantRentByVehicleTypeActivity.this.cldUntil.set(Calendar.MILLISECOND, 0);
                }
    ...
    private void widget() {
        ...
        this.recyclerView.setAdapter(new MerchantRentByVehicleTypeAdapter());
    ...
    private void dateChanged() {
        ...
        this.disposable = MerchantApi.rentByVehicleTypes(
                this,
