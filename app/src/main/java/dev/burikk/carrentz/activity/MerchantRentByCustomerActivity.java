...
import dev.burikk.carrentz.adapter.MerchantRentByCustomerAdapter;
...
public class MerchantRentByCustomerActivity extends AppCompatActivity implements MainProtocol<RentByCustomerResponse>, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        super.setContentView(R.layout.activity_merchant_rent_by_customer);
    ...
    @Override
    public void result(RentByCustomerResponse data) {
        MerchantRentByCustomerAdapter merchantRentByCustomerAdapter = (MerchantRentByCustomerAdapter) this.recyclerView.getAdapter();

        if (merchantRentByCustomerAdapter != null) {
            merchantRentByCustomerAdapter.getRentByCustomerItems().clear();
            merchantRentByCustomerAdapter.getRentByCustomerItems().addAll(data.getRentByCustomerItems());
            merchantRentByCustomerAdapter.notifyDataSetChanged();

            if (!merchantRentByCustomerAdapter.getRentByCustomerItems().isEmpty()) {
                this.notEmpty();
            } else {
                this.empty();
            }
...

        {
            ...
            for (RentByCustomerItem rentByCustomerItem : data.getRentByCustomerItems()) {
                pieEntries.add(new PieEntry(rentByCustomerItem.getAmount().floatValue(), rentByCustomerItem.getName(), rentByCustomerItem));
            }
        ...
        this.materialDatePicker.addOnPositiveButtonClickListener(selection -> {
            if (selection != null) {
                if (selection.first != null) {
                    MerchantRentByCustomerActivity.this.cldFrom.setTimeInMillis(selection.first);
                    MerchantRentByCustomerActivity.this.cldFrom.set(Calendar.HOUR, 0);
                    MerchantRentByCustomerActivity.this.cldFrom.set(Calendar.MINUTE, 0);
                    MerchantRentByCustomerActivity.this.cldFrom.set(Calendar.SECOND, 0);
                    MerchantRentByCustomerActivity.this.cldFrom.set(Calendar.MILLISECOND, 0);
                }

                if (selection.second != null) {
                    MerchantRentByCustomerActivity.this.cldUntil.setTimeInMillis(selection.second);
                    MerchantRentByCustomerActivity.this.cldUntil.set(Calendar.HOUR, 0);
                    MerchantRentByCustomerActivity.this.cldUntil.set(Calendar.MINUTE, 0);
                    MerchantRentByCustomerActivity.this.cldUntil.set(Calendar.SECOND, 0);
                    MerchantRentByCustomerActivity.this.cldUntil.set(Calendar.MILLISECOND, 0);
                }
    ...
    private void widget() {
        ...
        this.recyclerView.setAdapter(new MerchantRentByCustomerAdapter());
    ...
    private void dateChanged() {
        ...
        this.disposable = MerchantApi.rentByCustomers(
                this,
